...
Read when scoping a camera feature — this is the long-term target set to slice from, never a v1 requirement.
...
**Long-term target feature set** (per-feature Android implementation notes in [camera-feasibility-android.md](camera-feasibility-android.md)):
...
**How to apply:** this is the long-term target, never a v1 requirement — scope any new camera work as a small vertical slice from it (`CLAUDE.md`, "Minimal infrastructure"). Camera code follows the same module and layer conventions as everything else in `CLAUDE.md`; being the flagship feature does not earn it a bespoke structure. The interface half of the vision is expressed today as flat minimal chrome, not the 3D/skeuomorphic reading of "tactile" — see [design-direction.md](design-direction.md).
