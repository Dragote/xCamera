...
- `shared:diagnostics` is the sole place in the app that reads `CameraCharacteristics` for lens identity or capability purposes. `feature:camera` consumes its `LensSnapshot`, `LensEnumerator` and `CameraCapabilityChecks` rather than enumerating lenses itself.
