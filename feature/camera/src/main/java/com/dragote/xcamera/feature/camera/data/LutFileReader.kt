 ...
 * [readBytes] already covers by construction.
 *
 * Bytes, not text — `feature:settings`' `LutLocalDataSource` stores every LUT in
 * `com.dragote.xcamera.shared.common.domain.model.CubeLut.toBinary`'s compact binary format (a later
 * issue #43 follow-up, replacing the earlier ASCII `.cube`-text-on-disk approach this class's own
 * `readText` used to back), so there's no text content to read here anymore.
 *
